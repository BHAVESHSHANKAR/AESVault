...
// //         const { receiverId, senderId } = req.body;
// //         const file = req.file;
// //         if (!file || !receiverId || !senderId) return res.status(400).json({ error: "File, Sender ID, and Receiver ID are required" });
...
// //     const { userId, friendId } = req.body; // uniqueIds
// //     try {
...
// // Send OTP for File Upload (Updated to use receiverId)
// router.post("/send-upload-otp", async (req, res) => {
//     const { receiverId } = req.body; // Changed from email to receiverId
//     const user = await User.findOne({ uniqueId: receiverId }); // Lookup by uniqueId
//     if (!user) return res.status(400).json({ success: false, message: "Receiver not found!" });

//     const otp = Math.floor(100000 + Math.random() * 900000).toString();
//     await OTP.findOneAndUpdate(
//         { email: user.email }, // Use user's email internally
//         { otp, createdAt: new Date() },
//         { upsert: true, new: true }
//     );
